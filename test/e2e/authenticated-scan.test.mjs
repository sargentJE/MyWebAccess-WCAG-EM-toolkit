...
test(
  'authenticated scan: applyAuth.storageState lets Playwright pass /protected guard',
  { timeout: 60_000 },
  async (t) => {
    ...
    assert.equal(noAuthResponse?.status(), 401, 'without cookie, /protected must return 401');
    ...
    assert.ok(body.includes('Protected page'), 'protected page body must render with valid auth');
