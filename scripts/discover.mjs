...
import { normalizeUrl, guessPageType, clusterKeyFor, guessProcessTypes, urlAllowedByScope, urlExcludedByPatterns } from './lib/urls.mjs';
...
for (const url of await getSitemapSeeds(config.rootUrl, config.crawl.sitemapSeeding, config.scope)) {
  if (urlAllowedByScope(url, config.rootUrl, config.scope) && !urlExcludedByPatterns(url, config.crawl.excludeUrlPatterns)) {
    seeds.push(url);
  }
}
...
const crawler = new PlaywrightCrawler({
  ...
  async requestHandler({ page, request, enqueueLinks, log }) {
    ...
    const canonical = config.discovery.captureCanonical
      ...
      : null;
    const formCount = config.discovery.captureForms ? await page.locator('form').count().catch(() => 0) : 0;
    const landmarkCount = config.discovery.captureLandmarks
      ? await page.locator('main, nav, header, footer, aside, [role="main"], [role="navigation"], [role="banner"], [role="contentinfo"], [role="complementary"]').count().catch(() => 0)
      : 0;
    const searchInputCount = config.discovery.captureSearchInputs
      ? await page.locator('input[type="search"], input[role="searchbox"], form[role="search"] input').count().catch(() => 0)
      : 0;
...
const pageClusters = Object.values(Object.groupBy(inventory, item => item.clusterKey)).map(group => ({
...
const processCandidates = inventory
  ...
  .map(item => ({ url: item.url, processTypes: item.processTypes, pageType: item.pageType, formCount: item.formCount }));

await writeJson(path.join(inventoryDir, 'inventory.json'), inventory);
await writeText(path.join(inventoryDir, 'urls.txt'), inventory.map(item => item.url).join('\n') + '\n');
