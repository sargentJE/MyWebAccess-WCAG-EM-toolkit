...
const pageClusters = JSON.parse(
  await fs.readFile(path.join(inventoryDir, 'page-clusters.json'), 'utf8'),
);
...
if (fullSiteSupplementaryRecommended) {
  console.log(
    `Small-site note: full-site supplementary automated scan is recommended (inventoryCount=${inventoryCount}).`,
  );
}
